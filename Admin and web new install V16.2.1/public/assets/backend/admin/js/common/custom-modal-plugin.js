class CustomModalPlugin {
                ...
                <div class="modal-content shadow-lg">
                    ...
                    <div class="d-flex flex-column align-items-center text-center mb-30">
                        ...
                        <div class="d-flex justify-content-center gap-3 mt-3">
                            ${buttonHTML}
                            <button type="button" class="btn btn-secondary max-w-120 flex-grow-1" id="${this.modalId}-cancel-button" data-bs-dismiss="modal">Cancel</button>
    ...
    show({ image, title, message, confirmText, cancelText, onConfirm, customCallbacks }) {
        ...
        const cancelButton = document.getElementById(`${this.modalId}-cancel-button`);
        if (confirmText) okButton.innerText = confirmText;
        if (cancelText && cancelButton) cancelButton.innerText = cancelText;
        okButton.onclick = () => {
...
document.querySelector("body").addEventListener("click", function(event) {
    if (event.target.classList.contains("custom-modal-plugin")) {
        ...
        const onConfirmText = event.target.dataset.onConfirmText;
        const offConfirmText = event.target.dataset.offConfirmText;
        const onCancelText = event.target.dataset.onCancelText;
        const offCancelText = event.target.dataset.offCancelText;
        ...
        const confirmText = isChecked ? onConfirmText : offConfirmText;
        const cancelText = isChecked ? onCancelText : offCancelText;
