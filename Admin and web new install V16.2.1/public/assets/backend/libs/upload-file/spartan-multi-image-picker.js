...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function(options) {
        ...
        function addRow(settings, el){
            ...
            var placeholderImageWidth  = '100px';
            ...
            var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 0px; ">`+
                                `<div style="position: relative;">`+
                                    `<div class="spartan_item_loader" data-spartanindexloader="${count}" style=" position: absolute; width: 100%; height: ${rowHeight}; background: rgba(255,255,255, 0.7); z-index: 22; text-align: center; align-items: center; margin: auto; justify-content: center; flex-direction: column; display : none; font-size : 1.7em; color: #CECECE">` +
                                        `${uploadLoaderIcon}` +
                                    `</div>`+
                                    `<label class="file_upload" style="width: 100%; height: ${rowHeight}; border: 1px dashed #BDBDBE; border-radius: 5px; cursor: pointer; text-align: center; margin-top: 5px; margin-bottom : 5px; position : relative; display: flex; align-items: center; margin: auto; justify-content: center; flex-direction: column;">`+
                                        `<a href="javascript:" data-spartanindexremove="${count}" style="position: absolute !important; right : -6px; top: -6px; display : none; text-align: center; text-decoration : none; color : #ff0700;" class="spartan_remove_row"><i class="fi fi-sr-cross-circle"></i></a>`+
