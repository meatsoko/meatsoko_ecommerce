// Customer-facing auction AI assistant sidebar.
// Mirrors the admin sidebar behavior, adapted for the customer themes:
//  - CSRF token read from <meta name="_token"> (fallback to the form's _token input).
//  - Toasts use a toastMagic -> toastr -> console fallback (toastMagic may be absent).
//  - Post-image cascade re-uses the existing per-section generate buttons and waits
//    for the 'auction-ai-step-complete' events the auto-fill scripts emit.

// Resolve the CSRF token regardless of how the theme exposes it.
function getAuctionAiToken() {
...
function showAuctionAiSidebarError(message) {
...
// Extract human-readable messages from any backend error shape:
//  - errors: [{ error_code, message }]   (this app's validation/errorProcessor)
//  - errors: { field: ['msg', ...] }      (Laravel default)
//  - errors: ['msg', ...] / { field: 'msg' }
//  - message: 'msg'                        (exception/limit responses)
function extractAuctionAiErrors(xhr, fallbackMessage) {
    const messages = [];
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (modal) {
        ...
        // Bootstrap 5 dispatches a native event; the jQuery binding is a safe extra.
...
$(document).on('click', '#auctionAnalyzeImageBtn', function () {
    ...
    if (!imageInput || !imageInput.files[0]) {
        showAuctionAiSidebarError('Please select an image first');
        return;
    }
    ...
    $.ajax({
        ...
        contentType: false,
        headers: {
            'X-CSRF-TOKEN': getAuctionAiToken()
        },
        data: formData,
        success: function (response) {
            ...
            if (typeof generatedTitle !== 'string' || !generatedTitle.length) {
                showAuctionAiSidebarError('Failed to analyze image and generate auction title.');
                return;
            }
        ...
        error: function (xhr) {
            extractAuctionAiErrors(xhr, 'An unexpected error occurred during image analysis.').forEach(showAuctionAiSidebarError);
        },
        complete: function () {
            setTimeout(function () {
...
$(document).on('click', '#auctionGenerateTitleBtn', function () {
    ...
    if (!keywords || !keywords.trim().length) {
        showAuctionAiSidebarError('Please enter a keyword or prompt first');
        return;
    }
    ...
    $.ajax({
        ...
        error: function (xhr) {
            $generated.find('.show_generating_text').addClass('d-none');
            extractAuctionAiErrors(xhr, 'Failed to generate product titles.').forEach(showAuctionAiSidebarError);
        },
        complete: function () {
            $button.prop('disabled', false);
