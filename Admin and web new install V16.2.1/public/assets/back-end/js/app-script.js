...
$(document).on("ready", function () {
    ...
    // Hide any open daterangepicker when the page scrolls.
    // The picker is appended to <body> at fixed coordinates; once the layout
    // shifts the picker floats in the wrong place. Must call picker.hide()
    // (not jQuery .hide()) so the library's internal isShowing flag resets —
    // otherwise the next click is a no-op and the user must click twice.
    function hideOpenDateRangePickers() {
        var pickerInputs = $(
            '.js-daterangepicker, .js-daterangepicker-with-range, ' +
            '.js-daterangepicker-times-sec, .js-daterangepicker-times, ' +
            '.js-date-range-picker-only-times, .js-daterangepicker-time-only, ' +
    ...
    var dpScrolling = false;
    document.addEventListener('wheel', function (e) {
        if ($(e.target).closest('.daterangepicker').length) return;
        if (dpScrolling) return;
        dpScrolling = true;
        setTimeout(function () {
            hideOpenDateRangePickers();
            dpScrolling = false;
