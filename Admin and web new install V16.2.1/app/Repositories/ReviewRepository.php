...
class ReviewRepository implements ReviewRepositoryInterface
{
    ...
    public function getListWhere(array $orderBy = [], ?string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, ?int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->review->with($relations)
            ...
            ->when(isset($filters['from']) && !empty($filters['from']) && isset($filters['to']) && !empty($filters['to']), function ($query) use ($filters) {
                    return $query->whereBetween('created_at', [trim($filters['from']) . ' 00:00:00', trim($filters['to']) . ' 23:59:59']);
                }
            )->when(isset($filters['from']) && !empty($filters['from']) && (empty($filters['to']) || !isset($filters['to'])), function ($query) use ($filters) {
                    return $query->where('created_at', '>=', trim($filters['from']) . ' 00:00:00');
                }
            )->when(isset($filters['to']) && !empty($filters['to']) && (empty($filters['from']) || !isset($filters['from'])), function ($query) use ($filters) {
                    return $query->where('created_at', '<=', trim($filters['to']) . ' 23:59:59');
                }
