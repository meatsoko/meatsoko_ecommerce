...
class DeliveryManCashCollectController extends Controller
{
    ...
    public function list(Request $request, $id): JsonResponse
    {
        ...
        $deliveryMan = DeliveryMan::with(['wallet', 'transactions' => function ($query) {
            return $query->where(['transaction_type' => 'cash_in_hand']);
        }])->where('seller_id', $seller->id)->find($id);
        if (!$deliveryMan) {
            return response()->json(['message' => translate('invalid_deliveryman!')], 403);
        }

        $transactions = DeliveryManTransaction::where(['delivery_man_id' => $deliveryMan['id'], 'transaction_type' => 'cash_in_hand'])
            ->latest()
