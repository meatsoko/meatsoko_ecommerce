...
class PaymentController extends Controller
{
    ...
    public function getCustomerPaymentRequest(Request $request, $orderAdditionalData = []): mixed
    {
        ...
        // Snapshot the base-currency cart total so the payment-success webhook can
        // re-verify the live cart hasn't changed before an order is created from it.
        $additionalData['expected_cart_total'] = $paymentAmount;
    ...
    public function customerOrderEditPayDueAmount(Request $request)
    {
        ...
        $customer = Helpers::getCustomerInformation($request);
        $authenticatedOwnerId = $order->is_guest ? (int)($request['guest_id'] ?? session('guest_id')) : ($customer != 'offline' ? (int)$customer->id : null);
        if ($authenticatedOwnerId === null || (int)$order->customer_id !== $authenticatedOwnerId) {
            abort(403);
        }
