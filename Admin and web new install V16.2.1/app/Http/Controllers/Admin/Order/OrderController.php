...
class OrderController extends BaseController
{
    ...
    public function orderReturnAmountToCustomer(Request $request, CustomerWalletService $customerWalletService, OrderEditReturnAmountService $orderEditReturnAmountService): RedirectResponse
    {
        if (!Helpers::module_permission_check('orders')) {
            ToastMagic::error(translate('you_do_not_have_permission_to_perform_this_action'));
    ...
    ): JsonResponse
    {
        if (!Helpers::module_permission_check('orders')) {
            return response()->json([
                'status' => 0,
                'message' => translate('you_do_not_have_permission_to_perform_this_action'),
            ]);
        }
        ...
        $terminalOrderStatuses = ['delivered', 'canceled', 'returned', 'failed'];
        if (in_array($order['order_status'], $terminalOrderStatuses) && $order['order_status'] !== $request['order_status']) {
            return response()->json([
                'status' => 0,
                'message' => translate('order_status_can_not_be_changed_once') . ' ' . translate($order['order_status']),
            ]);
        }
