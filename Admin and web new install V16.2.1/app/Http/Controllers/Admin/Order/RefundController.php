...
class RefundController extends BaseController
{
    ...
    public function updateRefundStatus(RefundStatusRequest $request, RefundStatusService $refundStatusService, RefundTransactionService $refundTransactionService): JsonResponse
    {
        if (!Helpers::module_permission_check('orders')) {
            return response()->json(['error' => translate('you_do_not_have_permission_to_perform_this_action')]);
        }

        return DB::transaction(function () use ($request, $refundStatusService, $refundTransactionService) {
            $refund = RefundRequest::where('id', $request['id'])->lockForUpdate()->first();
            ...
            if (!$orderDetails) {
                return response()->json(['error' => translate('Order details not found for this refund.')]);
            }

            if ($request['refund_status'] == 'refunded') {
                if ($order['seller_is'] == 'admin') {
                    $adminWallet = $this->adminWalletRepo->getFirstWhere(params: ['admin_id' => $order['seller_id']]);
                    if (($adminWallet['inhouse_earning'] ?? 0) < $refund['amount']) {
                    ...
                    if (($sellerWallet['total_earning'] ?? 0) < $refund['amount']) {
                        return response()->json(['error' => translate('Insufficient wallet balance to process refund.')]);
                    }
