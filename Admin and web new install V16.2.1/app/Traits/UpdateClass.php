...
trait UpdateClass
{
    ...
    public function getProcessAllVersionsUpdates(): void
    {
        ...
        $this->getInsertDataOfVersion('16.3');
    }
    ...
    public function getInsertDataOfVersion($versionNumber): void
    {
        ...
        if ($versionNumber == '16.3') {
            $this->addOrUpdateAuctionFeatureInitData();
            $this->addOrUpdateAuctionNotificationMessages();
            ...
            if (Schema::hasColumn('withdrawal_methods', 'vendor_status')) {
            ...
            foreach (['taxables', 'order_taxes'] as $taxTable) {
                if (Schema::hasTable($taxTable) && Schema::hasColumn($taxTable, 'tax_for')) {
                    DB::table($taxTable)
                        ->where(function ($query) {
                            $query->whereNull('tax_for')->orWhere('tax_for', '');
                        })
                        ->update(['tax_for' => 'tax_for_orders']);
                }
            }
        }
    ...
    public function addOrUpdateAuctionFeatureInitData(): void
    {
        $this->businessSettingGetOrInsert(type: 'auction_feature_status', value: 0);
        $this->businessSettingGetOrInsert(type: 'auction_commission_percentage', value: 0);
        $this->businessSettingGetOrInsert(type: 'auction_entry_fee_amount_status', value: 0);
        $this->businessSettingGetOrInsert(type: 'auction_entry_fee_amount_value', value: 0);

        $this->businessSettingGetOrInsert(type: 'auction_expired_visibility_duration', value: json_encode([
            'status' => 0, 'value' => 0, 'unit' => 'day'
        ]));

        $this->businessSettingGetOrInsert(type: 'auction_upcoming_visibility_duration', value: json_encode([
            'status' => 0, 'value' => 0, 'unit' => 'day'
        ]));

        $this->businessSettingGetOrInsert(type: 'auction_winner_claim_time_limit', value: json_encode([
            'status' => 0, 'value' => 0, 'unit' => 'day'
        ]));
    }

    public function addOrUpdateAuctionNotificationMessages(): void
    {
        $customerMessages = [
            'auction_went_live' => 'An auction you joined is now live',
            'auction_outbid' => 'You have been outbid on an auction',
            'auction_won' => 'Congratulations! You won an auction',
            'auction_participation_payment_verified' => 'Your auction participation payment has been verified',
            'auction_claim_payment_verified' => 'Your auction claim payment has been verified',
            'auction_commission_payment_verified' => 'Your auction commission payment has been verified',
            ...
            'auction_claim_expired' => 'Your auction claim window has expired',
            'auction_next_bidder' => 'You can now claim the auction item',
            'auction_approved' => 'Your auction has been approved',
            'auction_denied' => 'Your auction has been denied',
            'auction_expired_result' => 'Your auction has ended',
            ...
            'auction_item_claimed' => 'Your auction item has been claimed',
            'auction_claim_payment_verified_owner' => 'The auction claim payment has been verified',
            'auction_withdrawal_approved' => 'Your auction withdrawal has been approved',
            'auction_withdrawal_rejected' => 'Your auction withdrawal has been rejected',
        ];
        ...
        foreach (['customer' => $customerMessages, 'seller' => $sellerMessages] as $userType => $messages) {
            foreach ($messages as $key => $message) {
                if (NotificationMessage::where(['key' => $key, 'user_type' => $userType])->exists()) {
                    continue;
                }
