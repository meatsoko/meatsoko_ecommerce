...
@section('profile_content')
    <div>
        <div class="d-flex flex-wrap align-items-center justify-content-end gap-3">
            <h4 class="text-capitalize flex-grow-1 mb-0 d-flex align-items-center gap-2">
                {{ translate('Transaction_with_Admin') }}
                <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill fs-12 fw-medium mt-1">{{ $transactions->total() }}</span>
            </h4>
                    ...
                    <form action="{{ route('auction.transaction-history') }}" method="get">
                        ...
                        <div class="d-flex justify-content-between align-items-center p-3 p-sm-4 border-bottom">
                            ...
                            <button id="filterCloseBtn" type="button" class="btn border-0 p-1">
                                <i class="fi fi-rr-cross d-flex text-dark"></i>
                            </button>
                        </div>
                        <div class="p-3 p-sm-4 overflow-auto max-h-290px">
                            <div class="bg-light p-3 rounded mb-4">
                                ...
                                <div class="bg-white p-3 rounded d-flex justify-content-between align-items-center flex-wrap gap-2">
                                    <div class="form-check flex-grow-1">
                                        <input type="checkbox" class="form-check-input form-check-input_theme" name="transaction_type[]" id="filterTxCredit" value="credit"
                                               {{ in_array('credit', $selectedTransactionTypes) ? 'checked' : '' }}>
                                        <label for="filterTxCredit" class="form-check-label mb-0 lh-24px title-clr">{{ translate('Credit') }}</label>
                                    </div>
                                    <div class="form-check flex-grow-1">
                                        <input type="checkbox" class="form-check-input form-check-input_theme" name="transaction_type[]" id="filterTxDebit" value="debit"
                                               {{ in_array('debit', $selectedTransactionTypes) ? 'checked' : '' }}>
                                        <label for="filterTxDebit" class="form-check-label mb-0 lh-24px title-clr">{{ translate('Debit') }}</label>
                                    </div>
                                </div>
                            </div>
                            <div class="bg-light p-3 rounded mb-0">
                                <div class="date-filter-wrapper">
                                    <div>
                                        <label for="" class="form-label mb-2">{{ translate('Date_Type') }}</label>
                        ...
                        <div class="card shadow-lg p-3 p-sm-4 d-flex flex-row gap-3">
                        ...
                        <div class="col-sm-6">
                            <div class="bg-light p-2 p-sm-4 rounded d-flex justify-content-between gap-3">
                                <div>
                                    ...
                                    <div class="fs-12 text-muted">{{ translate('Auction') }} # {{ $item->auction_product_id }}</div>
                                    @if($transactionTypeLabel)
                                        <div class="fs-12 text-muted mt-1">{{ $transactionTypeLabel }}</div>
                                    @endif
                                </div>
                                <div class="text-end">
                                    ...
                                    @if($item->type === 'debit')
                                        <p class="text-danger fs-14">{{ translate('Debit') }}</p>
                                    @else
                                        <p class="text-success fs-14">{{ translate('Credit') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
