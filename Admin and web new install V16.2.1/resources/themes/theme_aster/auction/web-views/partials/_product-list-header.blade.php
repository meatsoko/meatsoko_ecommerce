        ...
        <div class="d-flex flex-wrap gap-3 align-items-center justify-content-between">
            <div class="d-flex gap-lg-3 gap-2 align-items-center">
                @if(!empty($authorContext['url']))
                    <div class="rounded w-60px h-60px min-w-60px overflow-hidden text-center rounded-1 d-block flex-shrink-0">
                        <img src="{{ $authorContext['image'] }}" alt="{{ $authorContext['name'] }}" class="w-100 h-100 object-cover">
                    </div>
                @else
                    <div class="rounded w-60px h-60px min-w-60px overflow-hidden text-center rounded-1 flex-shrink-0">
                        <img src="{{ $authorContext['image'] }}" alt="{{ $authorContext['name'] }}" class="w-100 h-100 object-cover">
                    </div>
                @endif
                <div>
                    @if(!empty($authorContext['url']))
                        <div class="text-decoration-none">
                            <h3 class="mb-1 fs-18 text-capitalize fw-bold title-clr line--limit-1">{{ $authorContext['name'] }}</h3>
                        </div>
                    @else
                        <h3 class="mb-1 fs-18 text-capitalize fw-bold title-clr line--limit-1">{{ $authorContext['name'] }}</h3>
                    @endif
            ...
            @if($authorContext['owner_type'] !== 'customer')
                @if(auth('customer')->check())
                    <button type="button"
                        ...
                        <i class="fi fi-rr-comment fs-14"></i> {{ translate('Chat with Seller') }}
                    </button>
                @else
...
<div class="card border-0 shadow-sm mb-20">
    <div class="card-body">
        <div class="row g-md-3 g-2 align-items-center">
            ...
            <div class="col-md-9">
                <div class="d-flex flex-lg-nowrap flex-wrap gap-xxl-20 gap-2">
                    <div class="flex-grow-1">
                        <div class="d-flex gap-2 align-items-center max-w-400 ms-auto">
                            <div class="w-100 form-auction overflow-hidden rounded border d-flex align-items-center position-relative light-box search-header-product">
                                <input class="form-control rounded-0 border-0 product-list-filter-input auction-live-search-input" type="search" name="search" placeholder="{{ translate('Search_by_auction_name') }}" value="{{ request('search') }}" autocomplete="off">
                                <button class="btn btn-primary border-0 min-h-45 w-60px rounded-0 d-center" type="submit">
                    ...
                    <div class="min-h-45 w-md-100 d-flex align-items-center gap-10px border rounded ps-3">
                        ...
                        <div class="sorting-item w-md-100 d-flex align-items-center">
                            <label class="for-sorting mb-0 fs-13 text-nowrap">{{ translate('Sort By') }}:</label>
                            <select class="product-list-filter-input text-wrap border-0 shadow-none outline-0 rounded-0 fs-13 form-select" name="sort_by">
                                <option value="recent_created" {{ in_array(request('sort_by'), ['', 'recent_created'], true) ? 'selected' : '' }}>{{ translate('Default (Recent created)') }}</option>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="searchProduct-filer" aria-labelledby="searchProduct-filer">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title">{{ translate('Products Filter') }}</h5>
