...
<div class="ending-soon-card card-icon-support bg-white w-100 shadow-sm rounded position-relative h-100">
    ...
    <div class="m-thumbnail-wrap overflow-hidden ratio-1 text-center position-relative">
        <a href="{{ route('auction.product-details', ['slug' => $auctionProduct?->slug]) }}"
           class="m-thumbnail d-block h-100">
        ...
        <div
            class="countdown-time_box z-1 d-inline-flex bg-white shadow-sm flex-sm-nowrap flex-wrap gap-1 justify-content-between align-items-center py-5px px-2 lh-sm position-absolute">
    ...
    <div class="min-h-130px">
        <div class="light-box d-flex align-items-center justify-content-between gap-1 py-2 px-10">
            ...
            @if(auth('customer')->check())
                <button type="button" data-based-class="auction-saved-update-{{ $auctionProduct?->id }}" class="btn p-0 fs-16 outline-0 border-0 text-primary auction-saved-update auction-saved-update-{{ $auctionProduct?->id }} {{ $auctionProduct?->mySavedProduct ? 'is-saved' : '' }}" data-auction-product-id="{{ $auctionProduct->id }}">
                    <i class="fi fi-rr-bookmark text-primary icon-unsaved"></i>
                    <i class="fi fi-sr-bookmark text-primary icon-saved"></i>
                </button>
            @else
                <button type="button" class="btn p-0 fs-16 outline-0 border-0 text-primary" data-bs-toggle="modal" data-bs-target="#loginModal">
                    <i class="fi fi-rr-bookmark text-primary"></i>
                </button>
            @endif
        </div>
        <div class="py-10 bg-white px-10">
            <div class="d-flex justify-content-start flex-wrap gap-5px mb-12px align-items-center">
