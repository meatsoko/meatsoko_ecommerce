@forelse($bids as $index => $bid)
    ...
    <div class="{{ $rowClass }} d-flex w-100 align-items-center justify-content-between gap-4 p-10px">
        ...
        <div>
            <h6 class="d-flex text-title fw-semibold gap-5px align-items-center fs-14 justify-content-end">
...
@empty
    <div class="bg-light py-4 px-2 rounded text-center">
        <div>
            <img width="45" height="44" src="{{ dynamicAsset(path: 'public/assets/front-end/img/empty-icons/auction-pending-empty.svg') }}" alt="" class="mb-3"><br>
            <div class="m-0 fs-14 title-semidark">{{ translate('No bids placed yet.') }}</div>
        </div>
    </div>
@endforelse
