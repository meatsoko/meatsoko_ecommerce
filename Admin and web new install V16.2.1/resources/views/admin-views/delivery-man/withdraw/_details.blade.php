...
<div class="withdraw-info-sidebar d-flex justify-content-between flex-column">
    <div class="withdraw-details">
        ...
        <div class="d-flex flex-column align-items-center gap-1 mb-3">
            <h4 class="mb-3 text-capitalize">{{translate('withdraw_information')}}</h4>
        ...
        <div class="card mb-3">
            <div class="card-header">
                <h4 class="mb-0 font-medium text-capitalize font-weight-bold">{{translate('bank_info')}}</h4>
            </div>
            <div class="card-body">
        ...
        <div class="card mb-3">
            <div class="card-header">
                <h4 class="mb-0 font-medium text-capitalize font-weight-bold">{{translate('deliveryman_info')}}</h4>
            </div>
            <div class="card-body">
                <div class="key-val-list d-flex flex-column gap-2 min-width--60px">
                    <div class="key-val-list-item d-flex gap-3">
        ...
        @if($details['transaction_note'])
            <div class="card mb-3">
                <div class="card-header">
                    <h4 class="mb-0 font-medium text-capitalize font-weight-bold">{{translate(($details['approved'] == 0 ? 'pending':($details['approved'] ==1 ? 'approved' : 'denied' )).'_'.'Note')}}</h4>
                </div>
                <div class="card-body">
                    <div class="key-val-list d-flex flex-column gap-2 min-width--60px">
                        <div class="key-val-list-item d-flex gap-3">
        ...
        <form action="{{route('admin.delivery-man.withdraw-update-status',[$details['id']])}}" method="POST" id="approval-note-form" data-message="{{ translate('want_to_approve_this_withdraw_request') . '?' }}">
            ...
            <div class="mt-5 d-none note-area" id="approval-note" >
                ...
                <div class="d-flex gap-2 pt-3 justify-content-center">
                    <button type="button" class="btn btn-secondary min-w-120 back-to-details mx-2">
                        <i class="fi fi-rr-arrow-left me-1"></i> {{translate('back')}}
                    </button>
                    <button type="button" class="btn btn-primary min-w-120 form-submit" data-form-id="approval-note-form" data-message="">
                        <i class="fi fi-rr-paper-plane me-1"></i> {{translate('Submit')}}
                    </button>
                </div>
            </div>
        </form>
        <form action="{{route('admin.delivery-man.withdraw-update-status',[$details['id']])}}" method="POST" id="denial-note-form"  data-message="{{ translate('want_to_deny_this_withdraw_request') . '?' }}">
            ...
            <div class="mt-5 d-none note-area" id="denial-note">
                ...
                <div class="d-flex gap-2 pt-3 justify-content-center">
                    ...
                    <button type="button" class="btn btn-primary min-w-120 form-submit" data-form-id="denial-note-form" data-message="">
                        <i class="fi fi-rr-paper-plane me-1"></i> {{translate('Submit')}}
                    </button>
                </div>
            </div>
    ...
    <div class="mt-4 d-flex justify-content-center gap-3">
        <div class="withdraw-details">
            @if ($details['approved'] == 0)
                <button type="button" class="btn badge-danger text-bg-danger min-w-120 open-note mx-2" data-id="denial-note" data-message="{{translate('want_to_deny_this_withdraw_request').'?'}}">
                    <i class="fi fi-rr-cross-small me-1"></i> {{ translate('Deny') }}
                </button>
                <button type="button" class="btn btn-success min-w-120 open-note" data-id="approval-note" data-message="{{translate('want_to_approve_this_withdraw_request').'?'}}">
                    <i class="fi fi-rr-check me-1"></i> {{ translate('Approve') }}
                </button>
            @endif
        </div>
    </div>
